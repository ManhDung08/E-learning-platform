        ...
        <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: '#F4F6F8', minHeight: '100vh', fontFamily: 'Public Sans, sans-serif' }}>
            ...
            <Grid container spacing={3} mb={4}>
                <Grid item xs={12} sm={6} lg={3}>
                    <StatCard 
                        title="Doanh Thu" 
                        value={formatCompact(safeOverview?.revenue?.total)} 
                        icon={<AnalyticsIcon fontSize="large" />} 
                        color="#00A76F" bgColor="#D0F2E3" 
                    />
                </Grid>
                <Grid item xs={12} sm={6} lg={3}>
                    <StatCard 
                        title="Người Dùng" 
                        value={safeOverview?.users?.total || 0} 
                        icon={<GroupAddIcon fontSize="large" />} 
                        color="#006C9C" bgColor="#CAFDF5" 
                    />
                </Grid>
                <Grid item xs={12} sm={6} lg={3}>
                    <StatCard 
                        title="Khóa Học" 
                        value={safeOverview?.courses?.published || 0} 
                        icon={<SchoolIcon fontSize="large" />} 
                        color="#FFAB00" bgColor="#FFF5CC" 
                    />
                </Grid>
                <Grid item xs={12} sm={6} lg={3}>
                    <StatCard 
                        title="Chứng Chỉ" 
                        value={safeOverview?.certificates?.issued || 0} 
                        icon={<VerifiedIcon fontSize="large" />} 
                        color="#FF5630" bgColor="#FFE7D9" 
                    />
                </Grid>
            </Grid>
                        ...
                        <CardContent sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <Box flex={1} minHeight={0}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={processedRevenueData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                        <defs>
            ...
            <Grid container spacing={3}>
                                ...
                                >
                                    <Box display="flex" alignItems="center" gap={2}>
                                        <Avatar 
                                            ...
                                            src={course.image || ''} 
                                            alt={course.title}
                                            sx={{ width: 48, height: 48, borderRadius: 1.5, bgcolor: '#F4F6F8', color: '#637381', fontWeight: 700 }}
                                        >
                                            {idx + 1}
                                        </Avatar>
                                        <Box>
                                            <Typography variant="body2" fontWeight={700} color="#212B36" noWrap sx={{ maxWidth: 250 }}>
                                                {course.title}
                                            </Typography>
                                    ...
                                    <Typography variant="body2" fontWeight={700} color="#00A76F">
                                        {formatCompact(course.priceVND)} ₫
                                    </Typography>
                                </Box>
                            ))}
                            {(!safeCourses?.topCourses || safeCourses.topCourses.length === 0) && (
                                <Box p={4} textAlign="center"><Typography color="#637381">Chưa có dữ liệu</Typography></Box>
                            )}
                        </Stack>
                    </Card>
                </Grid>
                ...
                <Grid item xs={12} md={4}>
                    <Card sx={{ borderRadius: '12px', boxShadow: '0px 2px 10px rgba(0, 0, 0, 0.05)', height: '100%', minHeight: 400 }}>
                        <CardContent sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            {safeRevenue.topCourses && safeRevenue.topCourses.length > 0 ? (
                                <>
                                    <Box flex={1} minHeight={200}>
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie 
                                                    data={safeRevenue.topCourses?.slice(0, 4)} 
                                                    innerRadius={60} 
                                                    outerRadius={80} 
                                                    paddingAngle={2} 
                                                    dataKey="revenue" 
                                        ...
                                        {safeRevenue.topCourses?.slice(0, 3).map((entry, index) => (
                                            <Box key={index} display="flex" justifyContent="space-between" alignItems="center">
                                                <Box display="flex" alignItems="center" gap={1}>
                                                    ...
                                                    <Typography variant="body2" color="#637381" sx={{ maxWidth: 150 }} noWrap>
                                                        {entry.courseTitle}
                                                    </Typography>
                                                </Box>
                                                <Typography variant="body2" fontWeight={700} color="#212B36">
                                ...
                                <Box display="flex" alignItems="center" justifyContent="center" height="100%">
                                    <Typography color="#637381">Chưa có dữ liệu</Typography>
                                </Box>
                            )}
                        </CardContent>
                    </Card>

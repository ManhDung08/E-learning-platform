...
const InstructorsList = () => {
    const filters = [ "All Mentors", "Web Development", "Mobile App", "Data Science", "UI/UX Design", "Cyber Security" ];
  ...
  return (
    <div className='p-6'>
        <div className='flex flex-wrap gap-3 mb-8'>
            {filters.map((filter) => (
                <button key={filter} onClick={() => setActiveFilter(filter)}
                        className={`px-4 py-2.5 rounded-xl font-medium text-sm transition-colors shadow-md
                                    ${activeFilter === filter ? 'bg-[#97A87A] text-white ' : 'bg-white text-gray-600 hover:-translate-y-0.5 '}`}>
                    {filter}
                </button>
            ))}
        </div>
        ...
        <div className='flex justify-center items-center mt-12 space-x-2'>
            <button className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#97A87A] text-white">
                 &lt;
            </button>
            <span className='text-gray-500 font-medium'>Page</span>
            <button className='w-10 h-10 flex items-center justify-center rounded-lg bg-white border border-gray-200 text-gray-700 font-medium'>1</button>
